...
class CustomGallery {
    constructor() {
        ...
        this.prevButton.addEventListener('click', () => {
            ...
            this.changeSlide(-1);
            this.resetAutoSlide(); // Сбрасываем таймер автопереключения
        });
        ...
        // Инициализируем автопереключение слайдов
    ...
    // Добавляем методы для автопереключения слайдов
    startAutoSlide() {
        ...
        this.autoSlideInterval = setInterval(() => {
            // Проверяем, не идет ли сейчас анимация или интро-анимация
            if (!this.isAnimating && !this.isInitialAnimationPlaying) {
                this.params.direction = -1; // Переход к следующему слайду
                this.changeSlide(1);
            }
        }, 5000); // 5 секунд между переключениями
    }
